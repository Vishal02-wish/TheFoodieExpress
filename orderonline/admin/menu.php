						...
						<div class="card-body">
							...
							<div class="form-group">
								<div class="custom-control custom-switch">
									<input type="checkbox" name="status" class="custom-control-input" id="availability"
										checked>
							...
							<div class="form-group">
								<img src="<?php echo isset($image_path) ? '../assets/img/' . $cover_img : '' ?>" alt=""
									id="cimg">
							</div>
						</div>
								...
								while ($row = $cats->fetch_assoc()):
									...
									<tr>
										...
										<td class="">
											...
											<p>Description : <b class="truncate">
...
<script>
	...
	$('#manage-menu').submit(function (e) {
		...
		$.ajax({
			...
			success: function (resp) {
				if (resp == 1) {
					alert_toast("Manu has been updated successfully", 'success')
					setTimeout(function () {
						location.reload()
					},2000)
	...
	$('.delete_menu').click(function () {
		_conf("Do you want to delete ?", "delete_menu", [$(this).attr('data-id')])
	})
	function delete_menu($id) {
		...
		$.ajax({
			...
			success: function (resp) {
				if (resp == 1) {
					alert_toast("Manu deleted successfully", 'success')
					setTimeout(function () {
						location.reload()
					}, 2000)
